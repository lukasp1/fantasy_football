...
for currentGame in range(1, 1537):
		...
		for hourlyData in byHour.data:
			if count == 2:
				break
			if count == 0:
				currentTemp = str(hourlyData.temperature)
				currentHome = str(gameLocation[currentGame])
				currentAway = str(gameAway[currentGame])
				if len(currentTemp) < 3:
					currentTemp += ".00"
				if len(currentTemp) < 5:
					currentTemp += "0"
				if len(currentHome) < 3:
					currentHome += " "
				if len(currentAway) < 3:
					currentAway += " "
				
				target.write("TEMPERATURE: {}     HOME: {}     AWAY: {}     WEATHER CONDITIONS: {}".format(currentTemp, currentHome, currentAway, (byHour.icon))+ '\n')
			#if count == 1:
